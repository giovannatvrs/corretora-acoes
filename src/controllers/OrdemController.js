...
const OrdemController = {
  ...
  venderAcao: async (req, res) => {
    try {
      ...
      const resultado = await OrdemService.processarSolicitacaoVenda(
        idUsuario,
        codigoAcao,
        validacaoQtd.valor,
        tipoOrdem,
      ...
      if (error instanceof QuantidadeInsuficienteError || error.name === 'QuantidadeInsuficienteError') {
        return res.status(400).json({ error: error.message });
      }
